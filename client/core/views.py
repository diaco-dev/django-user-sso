...
def oauth_callback(request):
    """Handle OAuth callback from integrated_login"""
    ...
    try:
        ...
        # Send login request to integrated_login
        login_data = {
            'username': request.GET.get('username'),  # Assume username passed from client
            'password': request.GET.get('password'),  # Assume password passed from client
            'client_id': settings.CLIENT_ID,
            'client_secret': settings.CLIENT_SECRET,
            'redirect_uri': settings.REDIRECT_URI,
            'scope': 'openid profile email',
        ...
        logger.info(f"Sending login request with data: {login_data}")

        response = requests.post(
            "http://localhost:8000/integrated_login",
            data=login_data,
            headers={'Content-Type': 'application/x-www-form-urlencoded'}
        )

        if response.status_code != 200:
            logger.error(f"Login request failed: {response.text}")
            return HttpResponse(f"Login failed: {response.text}", status=400)
        ...
        <h1>OAuth Success!</h1>
        <p>Welcome, {tokens['user'].get('username', 'User')}!</p>
        <h3>User Info:</h3>
        <pre>{tokens['user']}</pre>
        <h3>Session Info:</h3>
